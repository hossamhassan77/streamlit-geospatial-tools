...
class GeoDataManipulator:
    ...
    def _get_uploaded_file(self):
        ...
        return st.selectbox(
            "Choose an option",
            [
                "https://raw.githubusercontent.com/hossamhassan77/streamlit-geospatial-tools/refs/heads/main/data/Neighborhood%20Names%20GIS.geojson",
                "https://raw.githubusercontent.com/hossamhassan77/streamlit-geospatial-tools/refs/heads/main/data/countries.csv",
    ...
    def _select_lat_long_columns(self):
        ...
        with col2:
            column_guess = self.data_frame.columns.str.contains(
                "lon|lng|long|longitude", case=False
            )
            lng_index = 0
            if len(self.data_frame.columns[column_guess].tolist()) != 0:
    ...
    def _add_geojson_layer(self, json_data_frame, layer_name):
        if "features" in json_data_frame and len(json_data_frame["features"]) > 0:
            first_feature = json_data_frame["features"][0]
            ...
            if first_feature.get("geometry", {}).get("type") == "Point":
                # Add as CircleMarkers
                for feature in json_data_frame["features"]:
                    coords = feature["geometry"]["coordinates"]
                    props = feature.get("properties", {})
                    popup_html = self.create_popup_html(props)
                    folium.CircleMarker(
                ...
                # Use existing GeoJson layer for other geometries
                folium.GeoJson(
                    self.data_frame,
                    name=layer_name,
                    zoom_on_click=True,
    ...
    def _display_layout(self):
        ...
        with col2:
            ...
            st.dataframe(self.data_frame.drop(columns="geometry"), width=700, height=500)
            st.download_button(
                label="Download data as CSV",
                data=self.data_frame.to_csv().encode("utf-8"),
                file_name=f"Streamlit_df{random.randint(1, 1000)}.csv",
                mime="text/csv",
            )
